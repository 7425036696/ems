...
export default function MessagesPage() {
  ...
  const handleReply = (message: any) => {
    ...
    const body = `\n\n--- On ${format(new Date(message.createdAt), "MMM d, yyyy, HH:mm")}, ${message.from.name
                ...
                <div className="max-h-[600px] overflow-y-auto">
                  {messages?.map((message: any) => (
                    <motion.div
                      ...
                      className={`cursor-pointer border-b border-border p-4 transition-colors hover:bg-accent ${selectedThread && selectedThread[0]?._id === message._id ? "bg-accent" : ""
