...
import { Building2, LayoutDashboard, Users, Clock, Calendar, Mail, LogOut, UserCog, FileText, X } from "lucide-react"
import { cn } from "@/lib/utils"
import { Button } from "./ui/button"
import { ThemeToggle } from "./theme-toggle"
import { showToast } from "@/lib/utils/toast"
import SidebarSkeleton from "./sidebar-skelton"
...
export function Sidebar({ isOpen = true, toggle, isMobile = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={cn(
          "fixed left-0 top-0 z-40 h-screen w-64 border-r border-border bg-card shadow-lg",
          // Removed hidden classes to allow animation
        )}
      >
        <div className="flex h-full flex-col relative">
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto p-4">
            {filteredNavItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => isMobile && toggle && toggle()}>
                  <motion.div
                    whileHover={{ x: 4 }}
                    whileTap={{ scale: 0.98 }}
                    className={cn(
